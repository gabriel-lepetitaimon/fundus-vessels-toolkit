...
class KernelBase:
    ...
    def approximate_weights(self, kernels: "torch.Tensor [n_out, n_in, h, w]", info=None, bias=False, ridge_alpha=0.1):
    ...
    def _prepare_conv(self, input, weight, stride, padding, dilation, transpose=False, output_padding=0):
        from ..utils.torch.convbn import compute_conv_outputs_dim, compute_padding
        ...
        if transpose:
            assert n_in == n_out, (
                "Incoherent number of input neurons between the provided input and transposed weight:\n"
                f"input.shape={input.shape} (n_in={n_in}), weight.shape={weight.shape} (n_in={n_out})."
            )
        else:
            assert n_in == n_in_w, (
                "Incoherent number of input neurons between the provided input and weight:\n"
                f"input.shape={input.shape} (n_in={n_in}), weight.shape={weight.shape} (n_in={n_in_w})."
            )
        assert k == k_w, (
            f"The provided weights have an incorrect number of kernels:\n "
        ...
        if output_padding not in (0, (0, 0)):
            if transpose:
                ...
                raise ValueError(
                    "Parameter output_padding should equal 0 for not transpose conv, "
    ...
    def composite_kernels_conv2d(
        self,
        input: "torch.Tensor [b,i,w,h]",
        weight: "np.array [n_out, n_in, k]",
        stride=1,
        padding="same",
        output_padding=0,
        dilation=1,
    ...
    # --- Preconvolve Base ---
    @staticmethod
    def preconvolve_base(
        input: "torch.Tensor [b,i,w,h]",
        base: "torch.Tensor [n_k, n, m]",
        stride=1,
        padding="same",
        output_padding=0,
        dilation=1,
        ...
        if not transpose:
            ...
            K = F.conv_tranpose2d(
                input, base, stride=stride, padding=pad, dilation=dilation, output_padding=output_padding
            )
    ...
    def preconvolved_base_conv2d(
        ...
        bases = KernelBase.preconvolve_base(
            input,
            self.base,
            stride=stride,
            padding=padding,
        ...
        assert n_in == n_in_w, (
            f"The provided inputs and weights have different number of input neuron:\n "
