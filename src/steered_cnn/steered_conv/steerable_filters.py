...
def max_steerable_harmonics(radius):
    ...
    inter_area = circle_area(radius + 0.5) - circle_area(radius - 0.5)
    return int(inter_area // 2) - 1


def radial_steerable_filter(size, k, r, std=0.5, oversampling=1, phase=0, normalize=False):
...
def plot_filter(F, axis=True, spd=False, plot=None, colorbar=False, vmax=None):
    ...
    if axis:
        ...
        ax_filt.tick_params(axis="both", which="both", length=0)
        # Fade axis color
        for spine in ax_filt.spines.values():
            spine.set_color("#ccc")

    # Minor ticks
    ax_filt.set_xticks(np.arange(-0.5, w, 1), minor=True)
    ax_filt.set_yticks(np.arange(-0.5, h, 1), minor=True)
