...
bool is_between(const TopoLabel& label, const TopoLabel& start, const TopoLabel& end, const bool& strict,
    ...
    if (label == start || label == end) return strict ? false : true;
...
#pragma omp parallel for
    for (int b = 0; b < B; ++b) {
        ...
        std::tie(branch_labels[b], branch_dir[b], branch_plausibility[b], tips_label[b], tips_rank[b]) =
            read_branch_topology(curve_acc, topo_idxs_acc, topo_labels_acc, topo_ranks_acc, fuzzy_skeleton_acc,
...
std::tuple<TopoLabel, float, float, std::array<TopoLabel, 2>, std::array<float, 2>> read_branch_topology(
    const Tensor2DAcc<int32_t>& curve_acc, const Tensor2DAcc<uint32_t>& topo_idxs,
    ...
    for (std::size_t n = 0; n < N; ++n) {
        const auto& topo_idx = topo_idxs[curve_acc[n][0]][curve_acc[n][1]];
        if (topo_idx != UINT32_MAX) curve.push_back(topo_idx);
    }
    if (curve.size() < N / 2) return {0, 0.0f, 0.0f, {0, 0}, {0.0f, 0.0f}};
    ...
    for (const auto& idx : curve)
        if (is_ancestor(main_ancestor, topo_labels[idx], true)) curveTmp.push_back(idx);
    if (curveTmp.size() < 3 || curveTmp.size() < curve.size() / 2) return {0, 0.0f, 0.0f, {0, 0}, {0.0f, 0.0f}};
    curve = curveTmp;
    ...
    if (curveTmp.size() >= 3) curve = curveTmp;
    ...
    for (const auto& [tip, idx] : {std::make_pair(0, curve.front()), std::make_pair(1, curve.back())}) {
        const auto& rank = topo_ranks[idx];
        if (rank < max_rank) {
