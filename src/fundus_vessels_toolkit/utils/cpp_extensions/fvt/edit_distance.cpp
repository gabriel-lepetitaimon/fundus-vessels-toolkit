...
std::tuple<torch::Tensor, IncidentBranchesMatch, torch::Tensor> nodes_similarity(
    ...
    torch::Tensor branch_uvector_2, bool rotation_invariant, int angle_dot_features) {
    ...
    TORCH_CHECK_VALUE(dot_features_1.ndimension() == 3, "dot_features_1 must be a 3D tensor: (N1, B1, F_dot),",
                      " instead of ", dot_features_1.sizes());
    TORCH_CHECK_VALUE(dot_features_2.ndimension() == 3, "dot_features_2 must be a 3D tensor: (N2, B2, F_dot),",
                      " instead of ", dot_features_2.sizes());
    ...
    TORCH_CHECK_VALUE(dot_features_1.size(0) == N1, "Invalid number of nodes for dot_features_1 1st dimension.",
                      " Expected ", N1, " but got ", dot_features_1.size(0), ".");
    TORCH_CHECK_VALUE(dot_features_2.size(0) == N2, "Invalid number of nodes for dot_features_2 1st dimension.",
                      " Expected ", N2, " but got ", dot_features_2.size(0), ".");
    ...
    const long F_dot = dot_features_1.size(2), F_l2 = scalar_features_1.size(2);
    TORCH_CHECK_VALUE(F_dot == dot_features_2.size(2),
                      "Incoherent number of features in dot_features_1: ", dot_features_1.sizes(),
                      " and dot_features_2: ", dot_features_2.sizes());
    ...
    const long maxB = dot_features_1.size(1);
    TORCH_CHECK_VALUE(maxB == dot_features_2.size(1),
                      "Incoherent number of branches in dot_features_1: ", dot_features_1.sizes(),
                      " and cos_features_2: ", dot_features_2.sizes());
    ...
    TORCH_CHECK_VALUE(maxB == scalar_features_2.size(1),
                      "Incoherent number of branches in dot_features_2: ", dot_features_2.sizes(),
                      " and scalar_features_2: ", scalar_features_2.sizes());
    if (branch_uvector_available) {
        TORCH_CHECK_VALUE(maxB == branch_uvector_1.size(1),
                          "Incoherent number of branches in dot_features_1 and branch_uvector_1");
        TORCH_CHECK_VALUE(maxB == branch_uvector_2.size(1),
                          "Incoherent number of branches in dot_features_2 and branch_uvector_2");
    }
    ...
    IncidentBranchesData data = {dot_features_1.accessor<float, 3>(),
                                 dot_features_2.accessor<float, 3>(),
...
std::tuple<float, std::vector<UIntPair>, uint> node_similarity(uint n1, uint n2, uint B1, uint B2,
    ...
    const uint F_dot = data.dot_features_1.size(2), F_l2 = data.scalar_features_1.size(2);

    // Sort the branches by ascending tangent direction
    std::vector<uint> b1_order(B1), b2_order(B2);
    std::iota(b1_order.begin(), b1_order.end(), 0);
    std::iota(b2_order.begin(), b2_order.end(), 0);
    std::sort(b1_order.begin(), b1_order.end(), [&](int i1, int i2) {
        return Point(data.branch_uvector_1[n1][i1]).pos_angle() < Point(data.branch_uvector_1[n1][i2]).pos_angle();
    });
    std::sort(b2_order.begin(), b2_order.end(), [&](int i1, int i2) {
        return Point(data.branch_uvector_2[n2][i1]).pos_angle() < Point(data.branch_uvector_2[n2][i2]).pos_angle();
    });
    ...
    auto node_match_weight = [&](uint b1, uint b2, const UIntPair& iniMatch) {
        ...
        auto const &f1 = data.dot_features_1[n1][b1], &f2 = data.dot_features_2[n2][b2];
        float cos_sim = 0;
        for (uint f = 0; f < F_dot; f++) cos_sim += f1[f] * f2[f];

        // Compute the match weight: 1 - AVERAGE(cosine similarities + L2 similarities)
        const float norm_sim = norm_similarity(b1, b2);
        float sim = 1 - (cos_sim + norm_sim) / (F_dot + F_l2);
        return sim < 1 ? sim : 1;
    };
    ...
    auto node_match_weight_invar = [&](uint b1, uint b2, const UIntPair& iniMatch) {
        ...
        auto const &f1 = data.dot_features_1[n1][b1], &f2 = data.dot_features_2[n2][b2];
        float cos_sim = 0;
        for (uint f = 0; f < (uint)data.angle_dot_features; f++) {
            const uint fy = f * 2, fx = f * 2 + 1;
            cos_sim += Point(f1[fy], f1[fx]).rotate(u_rot12).dot(Point(f2[fy], f2[fx]));
        }
        for (uint f = data.angle_dot_features * 2; f < F_dot; f++) cos_sim += f1[f] * f2[f];

        // Compute the match weight: 1 - AVERAGE(cosine similarities + L2 similarities)
        float sim = 1 - (cos_sim + norm_similarity(b1, b2)) / (F_dot - data.angle_dot_features + F_l2);
        return sim < 1 ? sim : 1;
    };
    ...
    if (rotation_invariant) {
        ...
        if (data.uvectors_available) {
            ...
            for (uint b1 = 0; b1 < B1; b1++) {
                if (abs(Point(data.branch_uvector_1[n1][b1]).angle()) < 10 * M_PI / 180) {
                    has_uncertain_branches = true;
                    break;
                }
            }
            for (uint b2 = has_uncertain_branches ? B2 : 0; b2 < B2; b2++) {
                if (abs(Point(data.branch_uvector_2[n2][b2]).angle()) < 10 * M_PI / 180) {
                    has_uncertain_branches = true;
                    break;
                }
            }
    ...
    if (rotation_invariant)
        return djikstra_optimal_branch_matching(b1_order, b2_order, node_match_weight_invar, initial_pairs, false);
    else
        return djikstra_optimal_branch_matching(b1_order, b2_order, node_match_weight, initial_pairs, true);
}
...
std::tuple<float, std::vector<UIntPair>, uint> djikstra_optimal_branch_matching(
    ...
    std::function<float(uint, uint, const UIntPair&)> eval_match_cost, std::vector<UIntPair> initial_pairs,
    const bool are_match_cost_independent_of_ini_pairs) {
    using namespace BranchMatching;

    // === INITIALIZE ===
    uint B1 = B1_order.size(), B2 = B2_order.size();
    ...
    auto true_branches_indexes = [&](DjikstraStep match) {
        ...
        return UIntPair{B1_order[(iniB1 + match.b1) % B1], B2_order[(iniB2 + match.b2) % B2]};
    };
