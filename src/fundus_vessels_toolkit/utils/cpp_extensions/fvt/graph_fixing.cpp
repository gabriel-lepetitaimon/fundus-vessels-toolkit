...
void remove_branches(std::vector<Edge> branchesToRemove,
  ...
  for (std::size_t i = 0; i < branchCurves.size(); i++) {
    if (itToRemove != branchesToRemove.end() && itToRemove->id == (int)i) {
      ...
      std::swap(branchCurves[nBranches], branchCurves[i]);
      for (auto const &p : branchCurves[nBranches])
        branchesLabelMap[p.y][p.x] = nBranches;
...
void remove_singleton_nodes(EdgeList &edge_list,
...
void remove_nodes(std::vector<std::size_t> nodesIdToRemove, EdgeList &edge_list,
                  std::vector<IntPoint> &nodeCoords,
                  Tensor2DAcc<int> &labelMap) {
