...
template <typename T>
T get_if_exists(const std::map<std::string, T>& map, const std::string& key,
...
struct Point {
  ...
  Point(const at::TensorAccessor<float, 1UL, at::DefaultPtrTraits, signed long>&
            yx);
...
#pragma omp declare reduction(merge : std::vector<Point> : omp_out.insert( \
        omp_out.end(), omp_in.begin(), omp_in.end()))
#pragma omp declare reduction(                                    \
        merge : std::vector<std::vector<Point>> : omp_out.insert( \
                omp_out.end(), omp_in.begin(), omp_in.end()))

#pragma omp declare reduction(merge : std::vector<IntPoint> : omp_out.insert( \
        omp_out.end(), omp_in.begin(), omp_in.end()))
#pragma omp declare reduction(                                       \
        merge : std::vector<std::vector<IntPoint>> : omp_out.insert( \
                omp_out.end(), omp_in.begin(), omp_in.end()))
#pragma omp declare reduction(merge : std::vector<int> : omp_out.insert( \
        omp_out.end(), omp_in.begin(), omp_in.end()))
...
#pragma omp declare reduction(                             \
        merge : std::vector<PointWithID> : omp_out.insert( \
                omp_out.end(), omp_in.begin(), omp_in.end()))
...
static const std::vector<float> GAUSSIAN_HALF_STD3 =
    gaussianHalfKernel1D(3, 10);

// === Average Filters ===
std::vector<float> movingAvg(const std::vector<float>& x, std::size_t size,
                             const std::vector<int>& evaluateAtID);
std::vector<float> movingAvg(const std::vector<float>& x,
...
std::vector<T> medianFilter(const std::vector<T>& x, int halfSize = 1) {
  ...
  for (int i = halfSize; i < (int)(x.size() - halfSize); i++) {
    auto sortedX =
        std::vector<T>(x.begin() + i - halfSize, x.begin() + i + halfSize + 1);
    std::sort(sortedX.begin(), sortedX.end());
...
std::vector<int> linspace_int(int start, int end, u_int n,
                              bool endpoint = true);
inline int linspace_int(int i, int start, int end, int n,
                        bool endpoint = true) {
  float step = (float)(end - start) / (endpoint ? (n - 1) : n);
  return (int)floor(start + i * step);
}
std::vector<double> linspace(double start, double end, u_int n,
                             bool endpoint = true);
inline double linspace(int i, double start, double end, int n,
                       bool endpoint = true) {
  double step = (end - start) / (endpoint ? (n - 1) : n);
  return start + i * step;
}

std::vector<std::size_t> arange(const std::size_t& start,
                                const std::size_t& end,
                                const std::size_t& step);

std::vector<int> quantize_triband(const std::vector<float>& x, float low,
                                  float high, std::size_t medianHalfSize = 3);
...
inline std::size_t matrix_index(const std::array<uint, N>& index,
...
template <typename T>
using Tensor1DAcc =
    at::TensorAccessor<T, 1UL, at::DefaultPtrTraits, signed long>;

template <typename T>
using Tensor2DAcc =
    at::TensorAccessor<T, 2UL, at::DefaultPtrTraits, signed long>;

template <typename T>
using Tensor3DAcc =
    at::TensorAccessor<T, 3UL, at::DefaultPtrTraits, signed long>;

template <typename T>
using Tensor4DAcc =
    at::TensorAccessor<T, 4UL, at::DefaultPtrTraits, signed long>;
...
template <typename T>
std::vector<torch::Tensor> vectors_to_tensors(
...
std::vector<CurveYX> tensors_to_curves(
...
#pragma omp declare reduction(merge : std::vector<Edge> : omp_out.insert( \
        omp_out.end(), omp_in.begin(), omp_in.end()))

GraphAdjList edge_list_to_adjlist(const std::vector<IntPair>& edges, int N = -1,
                                  bool directed = false,
                                  bool keep_orientation = true);
GraphAdjList edge_list_to_adjlist(const EdgeList& edges, int N = -1,
                                  bool directed = false);
GraphAdjList edge_list_to_adjlist(const Tensor2DAcc<int>& edges, int N = -1,
                                  bool directed = false);
 ...
 *
 */
static const std::array<PointWithID, 8> NEIGHBORHOOD = {{{-1, -1, 0},
                                                         {-1, 0, 1},
                                                         {-1, 1, 2},
                                                         {0, 1, 3},
                                                         {1, 1, 4},
 ...
 *
 */
static const std::array<PointWithID, 8> CLOSE_NEIGHBORHOOD = {{{-1, -1, 0},
                                                               {-1, 1, 2},
                                                               {1, 1, 4},
                                                               {1, -1, 6},
                                                               {-1, 0, 1},
...
static const std::array<std::array<int, 5>, 8> TRACK_NEXT_NEIGHBORS = {{
    {0, 1, 7, 2, 6},  // neighbor index of c relative to p is 0 (previous pixel
                      // is bottom-right)
    {1, 2, 0, 3,
     7},  // neighbor index of c relative to p is 1 (previous pixel is bottom)
    {2, 3, 1, 4, 0},  // neighbor index of c relative to p is 2 (previous pixel
                      // is bottom-left)
    {3, 4, 2, 5,
     1},  // neighbor index of c relative to p is 3 (previous pixel is left)
    {4, 5, 3, 6,
     2},  // neighbor index of c relative to p is 4 (previous pixel is top-left)
    {5, 6, 4, 7,
     3},  // neighbor index of c relative to p is 5 (previous pixel is top)
    {6, 7, 5, 0, 4},  // neighbor index of c relative to p is 6 (previous pixel
                      // is top-right)
    {7, 0, 6, 1,
     5}  // neighbor index of c relative to p is 7 (previous pixel is right)
}};
...
static const std::array<std::array<int, 5>, 8> TRACK_CLOSE_NEIGHBORS = {{
    {7, 1, 0, 6, 2},  // neighbor index of c relative to p is 0 (previous pixel
                      // is bottom-right)
    {1, 7, 3, 0,
     2},  // neighbor index of c relative to p is 1 (previous pixel is bottom)
    {1, 3, 2, 0, 4},  // neighbor index of c relative to p is 2 (previous pixel
                      // is bottom-left)
    {3, 1, 5, 2,
     4},  // neighbor index of c relative to p is 3 (previous pixel is left)
    {3, 5, 4, 2,
     6},  // neighbor index of c relative to p is 4 (previous pixel is top-left)
    {5, 3, 7, 4,
     6},  // neighbor index of c relative to p is 5 (previous pixel is top)
    {5, 7, 6, 4, 0},  // neighbor index of c relative to p is 6 (previous pixel
                      // is top-right)
    {7, 5, 1, 6,
     0}  // neighbor index of c relative to p is 7 (previous pixel is right)
}};
...
inline bool hit(uint8_t neighborhood, uint8_t positive_mask) {
  return (neighborhood & positive_mask) == positive_mask;
}

inline bool miss(uint8_t neighborhood, uint8_t negative_mask) {
  return (neighborhood & negative_mask) == 0;
}

inline bool hit_and_miss(uint8_t neighborhood, uint8_t positive_mask,
                         uint8_t negative_mask) {
  return (neighborhood & positive_mask) == positive_mask &&
         (neighborhood & negative_mask) == 0;
}
