 ...
 *
 * This method removes the invalid pixels at the tips of the branches from the
 * branchCurves and the branchesLabelMap.
 *
 * @param branchCurves A list of branches to clean defined by a vector of
 * points.
 * @param branchesLabelMap An accessor to a 2D tensor of shape (H, W) containing
 * the branch labels to clean.
 * @param segmentation An accessor to a 2D tensor of shape (H, W) containing the
 * vessels binary segmentation.
 * @param adjacency The adjacency list of the vessels graph.
 * @param maxRemovedLength The maximum number of pixels to remove from the
 * branch.
 * @param maxRemovedLengthEnd The maximum number of pixels to remove from the
 * branch tips connected to endpoints.
 * @param adaptativeTangent A boolean indicating if the tangent smoothing should
 * be scaled by the branch calibre.
 *
 * @return A list of pairs of vectors and floats representing the tangent and
 * the calibre of the branches tips.
 */
std::vector<std::array<std::tuple<Vector, float, IntPoint, IntPoint>, 2>>
clean_branches_skeleton(std::vector<CurveYX> &branchCurves,
                        Tensor2DAcc<int> &branchesLabelMap,
                        const Tensor2DAcc<bool> &segmentation,
                        const GraphAdjList &adjacency, int maxRemovedLength,
                        int maxRemovedLengthEnd, bool adaptativeTangent) {
  std::vector<IntPair> branches_tips(branchCurves.size(), {0, 0});
  std::vector<std::array<std::tuple<Vector, float, IntPoint, IntPoint>, 2>> out(
      branchCurves.size());
...
#pragma omp parallel for
  for (int nodeID = 0; nodeID < (int)adjacency.size(); nodeID++) {
    ...
    if (node_adjacency.size() == 1) {
      ...
      auto const &[tipIdx, tangent, calibre, boundL, boundR] =
          clean_branch_skeleton_tip(branchCurves, edge.id, startSide,
                                    segmentation, maxRemovedLengthEnd,
                                    adaptativeTangent);

      // ... and store the tip indices, in order to clean them later.
      branches_tips[edge.id][startSide ? 0 : 1] = tipIdx;
      out[edge.id][startSide ? 0 : 1] = {startSide ? tangent : -tangent,
                                         calibre, boundL, boundR};
    } else {
      //  ... **all** its incident branches...
      auto const &tips_around_node = clean_branch_skeleton_around_node(
          branchCurves, nodeID, node_adjacency, segmentation, maxRemovedLength,
          adaptativeTangent);
      ...
      for (auto const &edge : node_adjacency) {
        ...
        if (nodeID == edge.end) {
          branches_tips[edge.id][1] =
              tipIdx;  // TODO:  check why there was a +1 here...
...
#pragma omp parallel for
  for (std::size_t i = 0; i < branchCurves.size(); i++) {
    ...
    if (firstI <= lastI - 1) {
      // Remove the invalid pixels from the branch labels, the branchYX and the
      // tangents
      for (auto p = branchYX.begin() + lastI; p != branchYX.end(); p++)
        branchesLabelMap[p->y][p->x] = 0;
      branchYX.erase(branchYX.begin() + lastI + 1, branchYX.end());

      for (auto p = branchYX.begin(); p != branchYX.begin() + firstI; p++)
        branchesLabelMap[p->y][p->x] = 0;
...
bool is_valid_boundaries(const IntPoint &p,
                         const std::array<IntPoint, 2> &boundaries) {
 ...
 *
 * This methods find the part of the skeleton of the branches around a node
 * which should be removed. For each branch it identifies the first valid pixel
 * of the skeleton which satisfies the following conditions:
 *  - The pixel is roughly at the center of the branch.
 *  - The branch width is roughly the same for this pixel and the next one in
 * the skeleton.
 *  - The corresponding boundary pixels are closer to the current branch than to
 * any other.
 *
 *
 ...
 *
 */
std::vector<std::tuple<int, Vector, float, IntPoint, IntPoint>>
clean_branch_skeleton_around_node(const std::vector<CurveYX> &branchCurves,
                                  int nodeID,
  ...
  auto get_branch_start_end = [&](int branchID, bool forward) {
    ...
    const int curveEnd =
        (int)curveYX.size() - 1;  // Last valid index of the curve
    // If the node is a junction, it should be removed from the branch skeleton
    const int c0 = node_adjacency.size() > 1 ? 1 : 0;

    // The branch exploration should start by the last index if the node is at
    // the end of the branch
    const int start = forward ? c0 : curveEnd - c0;
    const int end = forward ? std::min(start + maxRemovedLength, curveEnd)
                            : std::max(start - maxRemovedLength, 0);
  ...
  auto find_first_valid_branch_pixel = [&](int branchI) {
    ...
    if (curveSize <= 3)
      return std::make_tuple(end, Point(0, 0), INVALID_CALIBRE,
                             IntPoint::Invalid(), IntPoint::Invalid());

    // === Check if pixel is valid (Lambda Function) ===
    auto is_branch_pixel_valid =
        [&](const IntPoint &p, const std::array<IntPoint, 2> &boundaries,
          ...
          // 3. Check if the branch width is constant for this pixel and the
          // next
          if (abs(calibre - nextCalibre) > 2.1213) return false;

          // If the node is an endpoint (it should not), the previous conditions
          // are enough to check its validity
          ...
          auto isClosestToCurrentBranch = [&](const IntPoint &bound) {
            ...
            for (auto [id, forward] : branchesInfos) {
              ...
              auto const [closest_p, dist] = find_closest_pixel(
                  otherCurve, bound, otherStart, otherEnd, true);
              if (dist < distToCurrentBranch) return false;
            }
            return true;
          };
          if (!isClosestToCurrentBranch(boundaries[0]) ||
              !isClosestToCurrentBranch(boundaries[1]))
            return false;

          return true;
        };

    Point tangent = fast_curve_tangent(curveYX, start, TANGENT_HALF_GAUSS,
                                       forward, !forward, 0, curveSize);
    auto boundaries =
        fast_branch_boundaries(curveYX, start, segmentation, tangent);
    float calibre = fast_branch_calibre(boundaries[0], boundaries[1], tangent);
    if (adaptativeTangent) {
      tangent = adaptative_curve_tangent(curveYX, start, calibre, forward,
                                         !forward, 0, curveSize);
      boundaries =
          fast_branch_boundaries(curveYX, start, segmentation, tangent);
    ...
    Point nextTangent = fast_curve_tangent(curveYX, nextI, TANGENT_HALF_GAUSS,
                                           forward, !forward, 0, curveSize);
    std::array<IntPoint, 2> nextBoundaries =
        fast_branch_boundaries(curveYX, nextI, segmentation, nextTangent);
    float nextCalibre =
        fast_branch_calibre(nextBoundaries[0], nextBoundaries[1], nextTangent);
    if (adaptativeTangent && nextCalibre == nextCalibre) {
      nextTangent = adaptative_curve_tangent(curveYX, nextI, nextCalibre,
                                             forward, !forward, 0, curveSize);
      nextBoundaries =
          fast_branch_boundaries(curveYX, nextI, segmentation, nextTangent);
      nextCalibre = fast_branch_calibre(nextBoundaries[0], nextBoundaries[1],
                                        nextTangent);
    }
    ...
    while (nextI != endI) {
      ...
      nextTangent2 = fast_curve_tangent(curveYX, nextI2, TANGENT_HALF_GAUSS,
                                        forward, !forward, 0, curveSize);
      nextBoundaries2 =
          fast_branch_boundaries(curveYX, nextI2, segmentation, nextTangent2);
      nextCalibre2 = fast_branch_calibre(nextBoundaries2[0], nextBoundaries2[1],
                                         nextTangent2);
      if (adaptativeTangent && nextCalibre2 == nextCalibre2) {
        nextTangent2 = adaptative_curve_tangent(
            curveYX, nextI2, nextCalibre2, forward, !forward, 0, curveSize);
        nextBoundaries2 =
            fast_branch_boundaries(curveYX, nextI2, segmentation, nextTangent2);
        nextCalibre2 = fast_branch_calibre(nextBoundaries2[0],
                                           nextBoundaries2[1], nextTangent2);
      }

      if (is_branch_pixel_valid(curveYX[nextI - inc], boundaries,
                                nextBoundaries2, tangent, nextTangent2, calibre,
                                nextCalibre2))
        return std::make_tuple(nextI - inc, tangent, calibre, boundaries[0],
                               boundaries[1]);
    ...
    return std::make_tuple(
        ...
        fast_branch_calibre(boundaries[0], boundaries[1], tangent),
        boundaries[0], boundaries[1]);
  };

  std::vector<std::tuple<int, Vector, float, IntPoint, IntPoint>> out(
      branchesInfos.size());
  for (std::size_t i = 0; i < branchesInfos.size(); i++)
    out[i] = find_first_valid_branch_pixel(i);
  return out;
}
 ...
 *
 * This methods find the part of the skeleton of a branch tip which should be
 * removed. It identifies the first valid pixel of the skeleton which satisfies
 ...
 * @param startTip A boolean indicating if the tip to clean is at the start or
...
std::tuple<int, Vector, float, IntPoint, IntPoint> clean_branch_skeleton_tip(
    const std::vector<CurveYX> &branchCurves, int branchID, bool startTip,
    const Tensor2DAcc<bool> &segmentation, int maxRemovedLength,
    bool adaptativeTangent = false) {
  const CurveYX &curveYX = branchCurves[branchID];
  int start = startTip ? 0 : curveYX.size() - 1;
  const int end = startTip
                      ? std::min(maxRemovedLength, (int)curveYX.size() - 2)
  ...
  if (std::abs(start - end) <= 3)
    return std::make_tuple(start,
                           Point(curveYX[end] - curveYX[start]).normalize(),
                           INVALID_CALIBRE, curveYX[start], curveYX[start]);

  if (end != (startTip ? (int)curveYX.size() - 2 : 1)) {
    const Point endTangent =
        fast_curve_tangent(curveYX, end, TANGENT_HALF_GAUSS);
    float refCalibre =
        fast_branch_calibre(curveYX, end, segmentation, endTangent);
    if (refCalibre == refCalibre)
      start +=
          std::min((int)std::floor(refCalibre / 2), std::abs(start - end)) *
          inc;
  }
  ...
  Point tangent = fast_curve_tangent(curveYX, start, TANGENT_HALF_GAUSS,
                                     startTip, !startTip, curveStart, curveEnd);
  auto boundaries =
      fast_branch_boundaries(curveYX, start, segmentation, tangent);
  float calibre = fast_branch_calibre(boundaries[0], boundaries[1], tangent);
  if (adaptativeTangent) {
    tangent = adaptative_curve_tangent(curveYX, start, calibre, startTip,
                                       !startTip, curveStart, curveEnd);
  ...
  for (int i = start; i != end; i += inc) {
    ...
    nextTangent = fast_curve_tangent(curveYX, nextI, TANGENT_HALF_GAUSS,
                                     startTip, !startTip, curveStart, curveEnd);
    nextBoundaries =
        fast_branch_boundaries(curveYX, nextI, segmentation, nextTangent);
    nextCalibre =
    ...
    if (adaptativeTangent) {
      nextTangent =
          adaptative_curve_tangent(curveYX, nextI, nextCalibre, startTip,
                                   !startTip, curveStart, curveEnd);
      nextBoundaries =
          fast_branch_boundaries(curveYX, nextI, segmentation, nextTangent);
      nextCalibre = fast_branch_calibre(nextBoundaries[0], nextBoundaries[1],
    ...
    if (is_branch_pixel_valid(curveYX[i], boundaries, nextBoundaries, tangent,
                              nextTangent, calibre, nextCalibre))
...
void remove_small_spurs(float min_length, EdgeList &edgeList,
                        std::vector<CurveYX> &branchCurves,
                        std::vector<IntPoint> &nodeCoords,
 ...
 *
 * @param branchCurves A list of branches skeleton defined by a vector of
 * points.
 * @param adjacency The adjacency list of the vessels graph.
 * @param segmentation An accessor to a 2D tensor of shape (H, W) containing the
 * vessels binary segmentation.
 * @param min_length The minimum length under which a terminal edges is
 * necessarily considered as a spur.
 * @param calibre_factor If positive, a spur is removed if its length is less
 * than the calibre ratio times the maximum calibre of adjacent branches.
 * @param max_length The maximum length of a spur: a spur longer than this is
 * never removed.
 *
 * @return A list of edges representing the terminal edges of the graph.
 */
std::vector<Edge> find_spurs(const std::vector<CurveYX> &branchCurves,
                             const EdgeList &edgeList,
                             const Tensor2DAcc<bool> &segmentation,
                             float min_length, const float calibre_factor,
...
#pragma omp parallel for reduction(merge : branchToRemove)
  for (auto const &edge : terminal_edge) {
    ...
    if (curveSize <= min_length) {
      ...
      // If the calibres are provided, compute the maximum spur length based on
      // the calibres
      float maxCalibre =
          std::max(largest_node_calibre(edge.start, adjacency, branchCurves,
                                        segmentation),
                   largest_node_calibre(edge.end, adjacency, branchCurves,
                                        segmentation));

      if (curveSize <= maxCalibre * calibre_factor)
        branchToRemove.push_back(edge);
    }
  }
...
float largest_node_calibre(const int nodeId, const GraphAdjList &adjacency,
                           const std::vector<CurveYX> &branchesCurves,
                           const Tensor2DAcc<bool> &segmentation) {
  float maxCalibre = -1;
  for (auto const &nearEdge : adjacency[nodeId]) {
    ...
    const float calibre =
        fast_branch_calibre(curve, evaluateAtI, segmentation, tangent);
