...
std::pair<int, IntPoint> track_nearest_branch(
    const IntPoint& start, const Point& direction, float angle,
  ...
  bool safe =
      start.is_inside(1, 1, H - 1, W - 1) &&
      cIter.leftRay().extrapolate(max_it + 1).is_inside(1, 1, H - 1, W - 1) &&
      ((cIter.rightRayDirection() * (max_it + 1)) + start)
          .is_inside(1, 1, H - 1, W - 1);
...
RayIterator::RayIterator(const IntPoint& start, Point direction)
    : point(start), error(0) {
...
RayIterator::RayIterator(const IntPoint& start, float delta,
...
ConeIterator::ConeIterator(const IntPoint& start, Point direction, float angle)
    : _start(start) {
...
bool ConeIterator::beyondRightRay(const IntPoint& p) {
  return _rightRay.cross(p - _start + Point(0.5, 0.5)) < 0;
}