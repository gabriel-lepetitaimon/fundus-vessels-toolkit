 ...
 */
SkeletonRank detect_skeleton_rank(const bool pixelValue,
                                  const uint8_t neighbors) {
  if (!pixelValue) {
    return hit(neighbors, 0b01010101) ? SkeletonRank::HOLLOW_CROSS
                                      : SkeletonRank::NONE;
  }
  ...
  for (int s = 0; s < 8; s++) {
    ...
    if (hit(neighbors_rolled, 0b10100100)) {
      if (miss(neighbors_rolled, 0b01010001) ||
          miss(neighbors_rolled, 0b01010010))
...
SkeletonRank detect_skeleton_rank_debug(const bool pixelValue,
                                        const uint8_t neighbors) {
  if (!pixelValue) {
    return hit(neighbors, 0b01010101) ? SkeletonRank::HOLLOW_CROSS
                                      : SkeletonRank::NONE;
...
void fix_hollow_crosses(Tensor2DAcc<int>& parsedSkel,
                        const std::vector<IntPoint>& hollow_crosses,
                        std::vector<IntPoint>& endpoints,
                        bool remove_single_endpoints) {
  for (const IntPoint& p : hollow_crosses) {
    for (const PointWithID& n : NEIGHBORHOOD) {  // Decrement neighbors
      if (parsedSkel[p.y + n.y][p.x + n.x] > 0)
        parsedSkel[p.y + n.y][p.x + n.x]--;
    }
  }
  ...
  for (const IntPoint& p : hollow_crosses) {
    ...
    if (removeThisCross) {
      ...
      if (remove_single_endpoints && point == SkeletonRank::ENDPOINT)
 ...
 *
 */
torch::Tensor detect_skeleton_nodes(torch::Tensor skeleton, bool fix_hollow,
                                    bool remove_single_endpoints) {
...
#pragma omp parallel for collapse(2) \
    reduction(merge : endpoints, hollow_crosses)
  for (int y = 1; y < H - 1; y++) {
    for (int x = 1; x < W - 1; x++) {
      const uint8_t neighbors = get_neighborhood(skeleton_accessor, y, x);
      const SkeletonRank point =
          detect_skeleton_rank((bool)skeleton_accessor[y][x], neighbors);
  ...
  // Fix Hollow Cross
  if (fix_hollow)
    fix_hollow_crosses(parsed_accessor, hollow_crosses, endpoints,
                       remove_single_endpoints);
...
#pragma omp parallel for
  ...
  return parsed_skeleton.index(
      {torch::indexing::Slice(1, -1), torch::indexing::Slice(1, -1)});
}
...
#pragma omp parallel for collapse(2)
  for (int y = 1; y < H - 1; y++) {
    for (int x = 1; x < W - 1; x++) {
      const uint8_t neighbors = get_neighborhood(skeleton_accessor, y, x);
      parsed_accessor[y][x] =
          detect_skeleton_rank_debug((bool)skeleton_accessor[y][x], neighbors);
    }
  }
 ...
 */
std::pair<std::vector<IntPoint>, std::vector<PointWithID>> get_node_neighbors(
    Tensor2DAcc<int> skeleton, IntPoint current, IntPoint shape) {
  std::vector<IntPoint> nodes;
  std::vector<PointWithID> branches;
  std::array<bool, 8> is_node = {false, false, false, false,
                                 false, false, false, false};

  for (auto it = CLOSE_NEIGHBORHOOD.begin(),
            end = CLOSE_NEIGHBORHOOD.begin() + 4;
       it != end; ++it) {
  ...
  for (auto it = CLOSE_NEIGHBORHOOD.begin() + 4, end = CLOSE_NEIGHBORHOOD.end();
       it != end; ++it) {
    const IntPoint n = *it + current;
    int n_id = it->id;

    if (!n.is_inside(shape) || is_node[prev_neighbor_id(n_id)] ||
        is_node[next_neighbor_id(n_id)])
      continue;
 ...
 *     - The coordinates of neighbor branches.
 */
std::pair<PointWithID, int> track_branch_neighbors(Tensor2DAcc<int> skeleton,
                                                   PointWithID current,
                                                   IntPoint shape) {
  ...
  for (const int& n_id : TRACK_NEXT_NEIGHBORS[current.id]) {
    ...
    } else if (nextBranchPoint.id == -1 &&
               pixelRole == PixelRole::BRANCH_ROLE) {
 ...
 */
std::tuple<EdgeList, std::vector<CurveYX>, std::vector<IntPoint>>
parse_skeleton_to_graph(torch::Tensor& skeletonMap) {
...
#pragma omp atomic capture
  ...
  for (const PointWithID& node : nodes) {
    ...
    auto const& [next_nodes, next_branches] =
        get_node_neighbors(skel_acc, {node.y, node.x}, shape);
    ...
    for (const PointWithID& n : next_branches) {
      if (label_acc[n.y][n.x] == 0) {
        ...
        while (label_acc[tracker.y][tracker.x] == 0) {
          ...
          std::tie(tracker, tracker_role) =
              track_branch_neighbors(skel_acc, tracker, shape);
