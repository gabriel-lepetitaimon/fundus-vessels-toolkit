...
IntPoint IntPoint::operator+(const IntPoint& p) const {
  return IntPoint(p.y + y, p.x + x);
}
Point IntPoint::operator+(const Point& p) const {
  return Point(y + p.y, x + p.x);
}
IntPoint IntPoint::operator-(const IntPoint& p) const {
  return IntPoint(y - p.y, x - p.x);
}
Point IntPoint::operator-(const Point& p) const {
  return Point(y - p.y, x - p.x);
}
...
bool IntPoint::is_inside(int h, int w) const {
  return (x >= 0 && x < w && y >= 0 && y < h);
}
bool IntPoint::is_inside(int y0, int x0, int y1, int x1) const {
  return (x >= x0 && x < x1 && y >= y0 && y < y1);
}
bool IntPoint::is_inside(const IntPoint& p) const {
...
Point::Point(
    const at::TensorAccessor<float, 1UL, at::DefaultPtrTraits, signed long>& yx)
    : y(yx[0]), x(yx[1]) {}
...
Point Point::operator+(const IntPoint& p) const {
...
bool Point::operator==(const IntPoint& p) const {
  return (x == p.x && y == p.y);
}
bool Point::operator!=(const Point& p) const { return (x != p.x || y != p.y); }
bool Point::operator!=(const IntPoint& p) const {
...
double Point::cosSim(const Point& p) const {
...
Point Point::positiveCoordinates() const {
  return Point(std::abs(y), std::abs(x));
}
...
/// @return The rotated point.
Point Point::rotate(const Point& u) const {
  return Point(y * u.x + x * u.y, x * u.x - y * u.y);
}
...
/// @param u A unitary vector.
/// @return The rotated point.
Point Point::rotate_neg(const Point& u) const {
  return Point(y * u.x - x * u.y, x * u.x + y * u.y);
}

bool Point::is_inside(double h, double w) const {
  return (x >= 0 && x < w && y >= 0 && y < h);
}
bool Point::is_inside(double y0, double x0, double y1, double x1) const {
  return (x >= x0 && x < x1 && y >= y0 && y < y1);
}
bool Point::is_inside(const Point& p) const {
  return (x >= 0 && x < p.x && y >= 0 && y < p.y);
}
bool Point::is_null() const { return (x == 0 && y == 0); }

IntPoint Point::toInt() const { return IntPoint((int)round(y), (int)round(x)); }
IntPoint Point::floor() const {
  return IntPoint((int)std::floor(y), (int)std::floor(x));
}
IntPoint Point::ceil() const {
  return IntPoint((int)std::ceil(y), (int)std::ceil(x));
}
...
bool PointWithID::operator==(const PointWithID& p) const {
...
bool PointWithID::isSamePoint(const IntPoint& p) const {
  return (x == p.x && y == p.y);
}
...
std::vector<float> movingAvg(const std::vector<float>& x,
                             const std::vector<float>& halfKernel) {
...
std::vector<float> movingAvg(const std::vector<float>& x, std::size_t size,
  ...
  for (std::size_t yI = 0; yI < ySize; yI++) {
    ...
    float sum = 0;
    int count = 0;
    for (std::size_t j = (i >= size ? i - size : 0);
         j <= (i + size < xSize ? i + size : xSize - 1); j++) {
      sum += x[j];
      count++;
    }
    y[yI] = sum / count;
...
float distanceSqr(const Point& p1, const Point& p2) {
  return pow(p1.y - p2.y, 2) + pow(p1.x - p2.x, 2);
}
...
std::vector<int> quantize_triband(const std::vector<float>& x, float low,
                                  float high, std::size_t medianHalfSize) {
  std::vector<int> y;
  y.reserve(x.size());
  for (const auto& v : x) {
    if (v < low)
...
torch::Tensor vector_to_tensor(
    const std::vector<std::array<IntPoint, 2>>& vec) {
  torch::Tensor tensor = torch::empty({(long)vec.size(), 2, 2}, torch::kInt32);
  auto accessor = tensor.accessor<int, 3>();
  for (int i = 0; i < (int)vec.size(); i++) {
...
torch::Tensor remove_rows(const torch::Tensor& tensor, std::vector<int> rows) {
  ...
  for (std::size_t i = 1; i < rows.size(); i++) {
    new_tensor.index_put_(
        {torch::indexing::Slice(rows[i - 1] + 1 - i, rows[i] - i, 1)},
...
CurveYX tensor_to_curve(const torch::Tensor& tensor) {
  ...
  for (int i = 0; i < tensor.size(0); i++)
    curveYX.push_back({accessor[i][0], accessor[i][1]});
  return curveYX;
}
...
std::vector<IntPair> tensor_to_vectorIntPair(const torch::Tensor& tensor) {
  ...
  for (std::size_t i = 0; i < (std::size_t)tensor.size(0); i++)
    vec.push_back({accessor[i][0], accessor[i][1]});
  return vec;
}
...
PointList tensor_to_pointList(const torch::Tensor& tensor) {
  ...
  for (std::size_t i = 0; i < (std::size_t)tensor.size(0); i++)
    curve.push_back({accessor[i][0], accessor[i][1]});
  return curve;
}
...
bool Edge::operator==(const Edge& e) const {
  return (start == e.start && end == e.end && id == e.id);
}
bool Edge::operator!=(const Edge& e) const {
  return (start != e.start || end != e.end || id != e.id);
}
...
GraphAdjList edge_list_to_adjlist(const std::vector<IntPair>& edges, int N,
                                  bool directed, bool keep_orientation) {
  if (N < 0) {
    N = -1;
...
GraphAdjList edge_list_to_adjlist(const Tensor2DAcc<int>& edges, int N,
                                  bool directed) {
  const std::size_t E = (std::size_t)edges.size(0);
  if (N < 0) {
    N = 0;
    for (std::size_t i = 0; i < E; i++)
      N = std::max(N, std::max(edges[i][0], edges[i][1]));
    N++;
  }
...
torch::Tensor edge_list_to_tensor(const EdgeList& edge_list) {
  torch::Tensor branches_list_tensor =
      torch::empty({(int)edge_list.size(), 2}, torch::kInt32);
