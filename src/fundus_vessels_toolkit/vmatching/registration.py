...
class TreeRegistrationResult:
    ...
    def inspect(
        ...
        *,
        ...
        label: Literal["node", "branch", "all", False] | None = "all",
        height=600,
        ...
        node_labels = label in ["node", "all"]
        edge_labels = label in ["branch", "all"]
        layer1 = draw_tree(tree1, view=m[0], node_labels=node_labels, edge_labels=edge_labels)
        ...
        layer2 = draw_tree(tree2, view=m[1], branch_color="av", node_labels=node_labels, edge_labels=edge_labels)
        ...
        for i, (b1s, b2s) in enumerate(zip(branch_match1, branch_match2, strict=False)):
            for i1, b in enumerate(b1s):
                branch_labels1[b] = str(i + 1) + ("abcdefghijklmnopqrstuvwxyz"[i1] if len(b1s) > 1 else "")
                branch_cmap1[int(b)] = cmap[i % len(cmap)]
            for i2, b in enumerate(b2s):
                branch_labels2[b] = str(i + 1) + ("abcdefghijklmnopqrstuvwxyz"[i2] if len(b2s) > 1 else "")
