...
class VGraphNode:
    ...
    def __init__(self, graph: VGraph, id: int):
        ...
        self._ibranch_ids: Int1DArray | None = None
        self._ibranch_dirs: Bool1DArray | None = None
    ...
    def attr(self) -> DFSetterAccessor:
        if not self.is_valid():
            raise RuntimeError("The node has been removed from the graph.")
        return DFSetterAccessor(self.__graph.node_attr, self._id)
    ...
    def _update_adjacent_branch_cache(self) -> tuple[Int1DArray, Bool1DArray]:
    ...
    @overload
    def tips_geodata(self, attrs: VBranchGeoDataKey, geodata: VGeometricData | int = 0) -> npt.NDArray: ...
    @overload
    def tips_geodata(
        self, attrs: Optional[list[VBranchGeoDataKey]] = None, geodata: VGeometricData | int = 0
...
class VGraphBranch:
    ...
    def __init__(self, graph: VGraph, id: int, nodes_id: Optional[IntPair] = None):
        self.__graph = graph
        self._id = id
        graph._branch_refs.add(self)

        self._node_ids: IntPair = graph._branch_list[id] if nodes_id is None else nodes_id
    ...
    def nodes(self) -> tuple[VGraphNode, VGraphNode]:
        ...
        n1, n2 = self._node_ids
        return VGraphNode(self.__graph, int(n1)), VGraphNode(self.__graph, int(n2))
    ...
    def attr(self) -> DFSetterAccessor:
        assert self.is_valid(), "The branch has been removed from the graph."
        return DFSetterAccessor(self.__graph.branch_attr, self._id)

    def curve(self, geodata: VGeometricData | int = 0) -> PointArray:
    ...
    @overload
    def tip_coord(self, first_tip: bool) -> Point: ...
    @overload
    def tip_coord(self, first_tip: None = None) -> tuple[Point, Point]: ...
    def tip_coord(self, first_tip: Optional[bool] = None) -> Point | tuple[Point, Point]:
        assert self.is_valid(), "The branch has been removed from the graph."
        geodata = self.graph.geometric_data()
        t = geodata.tip_coord(self._id, first_tip=first_tip)
        return (Point.from_array(t[0]), Point.from_array(t[1])) if first_tip is None else Point.from_array(t)

    @overload
    def tip_tangent(self, first_tip: bool) -> Point: ...
    @overload
    def tip_tangent(self, first_tip: None = None) -> tuple[Point, Point]: ...
    def tip_tangent(self, first_tip: Optional[bool] = None) -> Point | tuple[Point, Point]:
        assert self.is_valid(), "The branch has been removed from the graph."
        geodata = self.graph.geometric_data()
        t = geodata.tip_tangent(self._id, first_tip=first_tip)
        return (Point.from_array(t[0]), Point.from_array(t[1])) if first_tip is None else Point.from_array(t)
    ...
    def rasterize(
        ...
        if curve.size == 0 or (boundaries := geodata.branch_data(VBranchGeoData.Fields.BOUNDARIES, self._id)) is None:
            return (
...
class VGraph:
    ...
    def __init__(
        ...
        self._branch_list: IntPairArray = branch_list
    ...
    def __eq__(self, other: object) -> bool:
        def equals_df_or_none(df1: Optional[pd.DataFrame], df2: Optional[pd.DataFrame]) -> bool:
            ...
            if (df1 is None) or (df2 is None):
                return False
            return df1.equals(df2)  # type: ignore
        ...
        return (
            ...
            and equals_df_or_none(self._node_attr, other._node_attr)
            and equals_df_or_none(self._branch_attr, other._branch_attr)
        )
    ...
    def save(
        ...
        *,
        ...
        if self._node_attr is not None:
            data["nodes_attr"] = pandas_to_numpy_dict(self._node_attr)  # type: ignore
        if self._branch_attr is not None:
            data["branches_attr"] = pandas_to_numpy_dict(self._branch_attr)  # type: ignore
    ...
    def load(cls, filename: str | Path | NumpyDict, *, check_integrity: bool = True) -> Self:
        ...
        if isinstance(filename, (str, Path)):
            ...
            data = filename
        N = data.get("node_count", None)  # type: ignore
        if N is not None:
            N = int(N)  # type: ignore
        return cls(
    ...
    def _empty_like_kwargs(cls, other: Self) -> Dict[str, Any]:
        branch_attr = pd.DataFrame(columns=other._branch_attr.columns) if other._branch_attr is not None else None
        node_attr = pd.DataFrame(columns=other._node_attr.columns) if other._node_attr is not None else None
    ...
    def parse(cls, branch_list: str) -> Self:
        ...
        branch_list : str
            ...
            - Each branch is defined as ``n1->n2`` or ``n1➔n2`` where ``n1`` and ``n2`` are either the indices of the nodes connected by the branch or a label (starting with a letter).
        ...
        Examples
        --------
        >>> VGraph.parse("0->1 ; 3➔2➔1 ; 4").branch_list.tolist()
        [[0, 1], [3, 2], [2, 1]]

        >>> VGraph.parse("A➔B➔C ; B➔D").print_graph()
        'A➔B➔C ; B➔D'
        ...
        for branch in re.split(r";", branch_list):
            ...
            for n1, n2 in itertools.pairwise(consecutive_nodes):
                branches.append([node_idx(n1, branch), node_idx(n2, branch)])
        if node_type == "label":
            ...
            node_attr = None
        return cls(branches, node_count=node_count, check_integrity=False, node_attr=node_attr)

    def print_graph(self) -> str:
        """Print a simple text representation of the graph structure using the syntax of the parse method.

        Examples
        --------
        >>> VGraph.parse("0➔1➔2➔3 ; 1➔4 ; 2➔5").print_graph()
        '0➔1➔2➔3 ; 1➔4 ; 2➔5'

        >>> VGraph.parse("A➔B➔C ; B➔D").print_graph()
        'A➔B➔C ; B➔D'
        """
        if "_parsed_node_labels" in self.node_attr:
            node_labels = self.node_attr["_parsed_node_labels"].to_dict()
        else:
            node_labels = {i: str(i) for i in range(self.node_count)}

        repr_str = ""
        prev_n1 = None
        for i, (n0, n1) in enumerate(self._branch_list):
            if prev_n1 is None:
                repr_str += node_labels[n0]
            elif prev_n1 != n0:
                repr_str += f" ; {node_labels[n0]}"
            repr_str += f"➔{node_labels[n1]}"
            prev_n1 = n1
        return repr_str
    ...
    def subgraph(
        ...
        subgraph_branches: Bool1DArray = np.isin(self._branch_list, nodes).all(axis=1)  # type: ignore
        ...
        graph = VGraph(
            branch_list,
            geodata,
            self._node_attr.loc[nodes].reset_index(drop=True) if self._node_attr is not None else None,
            self._branch_attr.loc[subgraph_branches].reset_index(drop=True) if self._branch_attr is not None else None,
            len(nodes),
            check_integrity=False,
        )
    ...
    def geometric_data(self, id: Optional[int | VGeometricData] = None) -> VGeometricData:
        ...
        """
        if id is None:
            return self._geometric_data[0]
        elif isinstance(id, VGeometricData):
            assert id in self._geometric_data, "Invalid geometric data reference."
            return id
        assert 0 <= id < len(self._geometric_data), f"Invalid geometric data index {id}."
    ...
    def node_attr(self) -> pd.DataFrame:
        ...
        if self._node_attr is None:
            self._node_attr = pd.DataFrame(index=pd.RangeIndex(self.node_count))
        return self._node_attr

    @property
    def branch_attr(self) -> pd.DataFrame:
        """The attributes of the branches in the graph as a pandas.DataFrame."""
        if self._branch_attr is None:
            self._branch_attr = pd.DataFrame(index=pd.RangeIndex(self.branch_count))
        return self._branch_attr

    def as_node_ids(self, ids: NodeIndicesLike | None, /, *, check=True) -> Indices:
        ...
        if ids is None:
        ...
        elif np.issubdtype(type(ids), np.integer):
            if check and not (0 <= ids < self.node_count):  # type: ignore
            ...
            node_index = self._node_attr.index if self._node_attr is not None else pd.RangeIndex(self.node_count)
            if check and ids.index is not node_index:
                raise ValueError("Invalid node index: pd.Series must have the same index as the nodes attributes.")
            return node_index[ids].to_numpy()
        elif isinstance(ids, Sequence):
            ids_seq = [_.id if isinstance(_, VGraphNode) else _ for _ in ids]
        elif isinstance(ids, np.ndarray):
            ids_seq = ids
        else:
            raise ValueError("Invalid node index: not assimilable to node indices.")

        ids_array = np.atleast_1d(ids_seq).flatten()
    ...
    def as_branch_ids(
        self,
        ids: BranchIndicesLike | None,
        /,
        *,
        check=True,
        return_is_single: Literal[False] = False,
        filter: Literal["orphan", "endpoint", "non-endpoint"] | None = None,
    ) -> Indices: ...
    @overload
    def as_branch_ids(
        self,
    ...
    def as_branch_ids(
        ...
        *,
        ...
        check : bool, optional
            If True, check that the indices are valid. Default is True.

        return_is_single : bool, optional
            If True, return a boolean indicating if the input is a single index. Default is False.
        ...
        """  # noqa: E501
        if ids is None:
            branch_ids = np.arange(self.branch_count, dtype=int)
            single = False

        elif isinstance(ids, VGraphBranch):
            if check and not ids.is_valid():
                raise ValueError("Invalid branch index: the VGraphBranch references a deleted branch.")
            branch_ids = np.array([ids._id], dtype=int)
            single = True

        elif isinstance(ids, pd.Series):
            if ids.dtype != bool:
                raise ValueError("Invalid branch index: pd.Series must be a boolean mask.")
            branch_idx = self._branch_attr.index if self._branch_attr is not None else pd.RangeIndex(self.branch_count)
            if check and ids.index is not branch_idx:
                raise ValueError("Invalid branch index: pd.Series must have the same index as the branches attributes.")
            branch_ids = branch_idx[ids].to_numpy()
            single = False

        elif np.isscalar(ids):
            if check:
                if not np.issubdtype(type(ids), np.integer):
                    raise ValueError("Invalid branch index: must be an integer.")
                if not (0 <= ids < self.branch_count):  # type: ignore
                    raise ValueError("Invalid branch index: out of range")
            branch_ids = np.array([ids], dtype=int)
            single = True
        else:
            if isinstance(ids, Sequence):
                ids_seq = [_.id if isinstance(_, VGraphBranch) else _ for _ in ids]
            elif isinstance(ids, np.ndarray):
                ids_seq = ids
            else:
                raise ValueError("Invalid node index: not assimilable to node indices.")
            ...
            if ids_array.dtype == bool:
                ...
                branch_ids = np.argwhere(ids_array).flatten()
                single = False
            else:
                branch_ids = ids_array.astype(int)
                if check and not np.all(np.logical_and(0 <= branch_ids, branch_ids < self.branch_count)):
                    raise ValueError("Invalid branch index: out of range.")
                single = False

        if filter is not None:
            if filter == "orphan":
                branch_ids = np.intersect1d(branch_ids, self.orphan_branches())
            elif filter == "endpoint":
                branch_ids = np.intersect1d(branch_ids, self.endpoint_branches())
            elif filter == "non-endpoint":
                branch_ids = np.setdiff1d(branch_ids, self.endpoint_branches())

        return (branch_ids, single) if return_is_single else branch_ids
    ...
    ) -> Int1DArray | tuple[Int1DArray, Bool1DArray]:
    ...
    def adjacent_nodes(self, node_id: NodeIndicesLike) -> Int1DArray:
        ...
        return np.setdiff1d(adjacents, node_id, assume_unique=True)  # type: ignore
    ...
    @overload
    def adjacent_branches_per_node(
        self, node_id: Optional[NodeIndicesLike] = None, *, return_branch_direction: Literal[True]
    ...
    def _adjacent_branches(
        self, node_ids: NodeIndicesLike, return_branch_direction: Literal[True], individual_nodes: Literal[True]
    ) -> tuple[list[Int1DArray], list[list[Bool1DArray]]]: ...
    @overload
    def _adjacent_branches(
        self, node_ids: NodeIndicesLike, return_branch_direction: Literal[False], individual_nodes: Literal[True]
    ) -> list[Int1DArray]: ...
    @overload
    def _adjacent_branches(
        self, node_ids: NodeIndicesLike, return_branch_direction: Literal[True], individual_nodes: Literal[False]
    ) -> tuple[Int1DArray, Bool1DArray]: ...
    @overload
    def _adjacent_branches(
        self, node_ids: NodeIndicesLike, return_branch_direction: Literal[False], individual_nodes: Literal[False]
    ) -> Int1DArray: ...
    def _adjacent_branches(
        self, node_ids: NodeIndicesLike, return_branch_direction: bool, individual_nodes: bool
    ) -> (
        list[Int1DArray]
        | tuple[list[Int1DArray], list[list[Bool1DArray]]]
        | Int1DArray
        ...
        if not individual_nodes:
            if node_branches.shape[0] == len(node_ids) and np.all(node_branches[:, 0] == np.arange(len(node_ids))):
                branch_ids: Int1DArray = node_branches[:, 1]  # type: ignore
            else:
                branch_ids = np.unique(node_branches[:, 1])  # type: ignore
            if return_branch_direction:
                return branch_ids, np.isin(self._branch_list[branch_ids][:, 0], node_ids[:, None])  # type: ignore
            return branch_ids
    ...
    @overload
    def endpoint_nodes(self, as_mask: Literal[False] = False) -> Int1DArray: ...
    @overload
    def endpoint_nodes(self, as_mask: Literal[True]) -> Bool1DArray: ...
    def endpoint_nodes(self, as_mask=False) -> Int1DArray | Bool1DArray:
    ...
    def endpoint_nodes_with_branch_id(
        ...
        Returns
        -------
        nodes_index: npt.NDArray[np.int32]
            The indices of the endpoints nodes.

        incident_branch_index: npt.NDArray[np.int32]
    ...
    def passing_nodes(self, *, as_mask=False, exclude_loop: bool = True) -> Indices | Bool1DArray:
        ...
        Returns
        -------
        passing_nodes_index: Indices | Bool1DArray
    ...
    def endpoint_branches(
        self, *, as_mask: Literal[False] = False, return_node_mask: Literal[False] = False
    ) -> Indices: ...
    @overload
    def endpoint_branches(self, *, as_mask: Literal[True], return_node_mask: Literal[False] = False) -> Bool1DArray: ...
    @overload
    def endpoint_branches(
        self, *, as_mask: Literal[False] = False, return_node_mask: Literal[True]
    ) -> tuple[Indices, Bool2DArray]: ...
    @overload
    def endpoint_branches(
        self, *, as_mask: Literal[True], return_node_mask: Literal[True]
    ) -> tuple[Bool1DArray, Bool2DArray]: ...
    def endpoint_branches(
        self, *, as_mask=False, return_node_mask=False
    ) -> Indices | Bool1DArray | tuple[Bool1DArray, Bool2DArray] | tuple[Indices, Bool2DArray]:
        ...
        Returns
        -------
        endpoints_branches : npt.NDArray[np.int32 | np.bool_]
        ...
        endpoints_mask: Bool2DArray = endpoint_nodes[self._branch_list]  # type: ignore
        endpoints_branches = np.any(endpoints_mask, axis=1)
        if not as_mask:
            endpoints_branches = np.argwhere(endpoints_branches).flatten()
            endpoints_mask = endpoints_mask[endpoints_branches]  # type: ignore
    ...
    def orphan_nodes(self) -> Indices:
        """Return the indices of the nodes connected to no branch in the graph.

        Parameters
        ----------
        as_mask : bool, optional
            If True, return a mask of the orphan nodes instead of their indices.

        Returns
        -------
        np.ndarray
            The indices of the nodes connected to no branch.

        Examples
        --------
        >>> # Branch id:           0 1        3
        >>> graph = VGraph.parse("0➔1➔2 ; 3; 4➔4")

        >>> graph.orphan_nodes()
        array([3])

        """
        connected_nodes = np.unique(self._branch_list.flatten())
        return np.setdiff1d(np.arange(self.node_count, dtype=int), connected_nodes, assume_unique=True)

    @overload
    def orphan_branches(self, as_mask: Literal[False] = False) -> Indices: ...
    @overload
    def orphan_branches(self, as_mask: Literal[True]) -> Bool1DArray: ...
    def orphan_branches(self, as_mask=False) -> Indices | Bool1DArray:
    ...
    @overload
    def self_loop_branches(self, as_mask: Literal[False] = False) -> Indices: ...
    @overload
    def self_loop_branches(self, as_mask: Literal[True]) -> Bool1DArray: ...
    def self_loop_branches(self, as_mask=False) -> Indices | Bool1DArray:
    ...
    def twin_branches(self) -> list[Indices]:
    ...
    def subgraph_branch_labels(self) -> Int1DArray:
        ...
        -------
        np.ndarray
            An array of shape (B,) containing the subgraph label of each branch in the graph.

        Examples
        --------
        >>> graph = VGraph.parse("0➔1➔2➔3 ; 4➔4 ; 5➔6")

        >>> graph.subgraph_branch_labels()
        array([0, 0, 0, 2, 1])
        """
        if self.branch_count == 0:
            return np.empty((0,), dtype=int)

        cc = self.node_connected_components()
        branch_labels = np.full((self.branch_count,), -1, dtype=int)
        for label, nodes in enumerate(cc):
            branch_labels[self.adjacent_branches(nodes)] = label
    ...
    def node_coord(self) -> FloatPairArray:
        ...
        coord = np.zeros((self.node_count, 2), dtype=np.float64)
    ...
    def _geometrical_data_priority(
        ...
        if np.isscalar(geometrical_data_priority) and np.issubdtype(type(geometrical_data_priority), np.integer):
    ...
    def append(self, other: VGraph, *, inplace=False) -> Self | tuple[Self, npt.NDArray[np.int32]]:
        ...
        >>> graph1.append(graph2).print_graph()
        'A1➔B1➔C1 ; A2➔B2'
        ...
        # Update attributes
        def empty_if_none(df: Optional[pd.DataFrame], length) -> pd.DataFrame:
            return pd.DataFrame(index=pd.RangeIndex(length)) if df is None else df

        if self._node_attr is not None or other._node_attr is not None:
            self_node_attr = empty_if_none(self._node_attr, self.node_count)
            other_node_attr = empty_if_none(other._node_attr, other.node_count)
            self._node_attr = pd.concat([self_node_attr, other_node_attr], ignore_index=True)

        if self._branch_attr is not None or other._branch_attr is not None:
            self_branch_attr = empty_if_none(self._branch_attr, self.branch_count)
            other_branch_attr = empty_if_none(other._branch_attr, other.branch_count)
            self._branch_attr = pd.concat([self_branch_attr, other_branch_attr], ignore_index=True)

        # Update geometric data
        if len(self._geometric_data) == 0:
            self._geometric_data = other._geometric_data.copy()
        else:
    ...
    def reindex_nodes(
        ...
        graph._branch_list = indices[graph._branch_list]  # ... branch list # type: ignore
        if graph._node_attr is not None:
            graph._node_attr = graph._node_attr.set_index(indices)  # ... nodes attributes # type: ignore
    ...
    def flip_branch_direction(self, branch_id: BranchIndicesLike, inplace=False) -> Self:
        ...
        for branch_ref in graph._branch_refs:  # ... branches references
            if branch_ref._id in branches_id:
                branch_ref._node_ids = graph._branch_list[branch_ref._id]
        for node_ref in graph._node_refs:  # ... incident branches stored in nodes references
            if (
                ...
                node_ref._ibranch_dirs[np.isin(node_ref._ibranch_ids, branches_id)] ^= True  # Flip direction
        return graph

    def sort_branches_by_nodesID(self, descending=False, inplace: bool = False) -> Self:
        ...
        Examples
        --------
        >>> graph = VGraph.parse("2➔3 ; 1➔0 ; 3➔4")
        >>> graph.sort_branches_by_nodesID().print_graph()
        '0➔1 ; 2➔3➔4'
        """
    ...
    def _delete_branch(self, branch_id: Indices, update_refs: bool = True) -> Indices:
        if len(branch_id) == 0:
    ...
    def _delete_node(self, node_id: Indices, update_refs: bool = True) -> Indices:
        if len(node_id) == 0:
        ...
        if update_refs:
            ...
            for branch in self._branch_refs:
                if branch.is_valid():
                    branch._node_ids = self._branch_list[branch._id]

        return nodes_reindex

    # --- Branches edition ---
    def delete_branch(self, branch_id: BranchIndicesLike, delete_orphan_nodes=True, *, inplace=False) -> Self:
        ...
        --------
        >>> # Branch id:           0 1     2 3     4
        >>> graph = VGraph.parse("A➔B➔C ; A➔D➔E ; F➔F")
    ...
    def duplicate_branch(
        self, branch_id: BranchIndicesLike, *, return_branch_id: Literal[False] = False, inplace=False
    ) -> Self: ...
    @overload
    def duplicate_branch(
        self, branch_id: BranchIndicesLike, *, return_branch_id: Literal[True], inplace=False
    ) -> tuple[VGraph, npt.NDArray[np.int32]]: ...
    def duplicate_branch(
        self, branch_id: BranchIndicesLike, *, return_branch_id=False, inplace=False
    ...
    def split_branch(
        ...
        *,
        ...
        _, nEnd = graph._branch_list[branch_id]
        ...
        for branch in graph._branch_refs:
            if branch._id == branch_id:
                branch._node_ids = graph._branch_list[branch_id]

        if return_branch_ids:
            branch_ids = np.concatenate([[branch_id], new_branchIds])
    ...
    def split_node(
        ...
        if len(branch_clusters) < 2:
            return graph if not return_node_ids else (graph, np.array([node_id], dtype=np.int_))
    ...
    def delete_node(self, node_id: NodeIndicesLike, *, inplace=False) -> Self:
        ...
        is_orphan = np.isin(node_id, np.unique(self._branch_list), invert=True)
        node_lookup = self._delete_node(node_id[is_orphan])

        node_id = node_lookup[node_id[~is_orphan]]
    ...
    def _fuse_nodes(
        self,
        node_id: Int1DArrayLike,
        *,
        quietly_ignore_invalid_nodes: Optional[bool] = False,
        incident_branches: Optional[IntPairArrayLike] = None,
    ) -> tuple[list[npt.NDArray[np.int32]], npt.NDArray[np.bool_], npt.NDArray[np.int32], npt.NDArray[np.int32]]:
    ...
    def _merge_consecutive_branches(
        self, branch_pairs: IntPairArrayLike, junction_nodes: Int1DArrayLike, remove_orphan_nodes: bool = True
    ) -> tuple[list[Indices], Bool1DArray, Indices, Indices]:
        ...
        Returns
        -------
        consecutive_branches : list[Indices]
            The list of the N clusters of merged branches. The first branch now contains all the branches of the group. The index correspond the branches indices before the merge.

        main_branch_flipped : Bool1DArray
            A boolean array of shape (N,) indicating if the main branch of each group was flipped.

        branch_merge_lookup : Indices
            A lookup table to reindex the branches after the merge.

        branches_to_delete : Indices
            The indices of the branches that were deleted. The index correspond the branches indices before the merge.
        """  # noqa: E501
        ...
        chains = reduce_chains(branch_pairs.tolist(), return_index=True)
        consecutive_branches = [np.array(c, dtype=np.int_) for c in chains[0]]
        consecutive_pair_ids = [np.array(c, dtype=np.int_) for c in chains[1]]
    ...
    def merge_nodes(
        ...
        *,
        ...
        return_branch_reindex_lookup : bool, optional
            If True, return a lookup table to reindex the branches after the merge. The index correspond the branches indices before the merge. The branches that are removed are set to -1.
        ...
        nodes_to_remove = []
        resulting_nodes = []
        nodes_lookup = np.arange(graph.node_count, dtype=int)

        if not assume_reduced:
            clusters = reduce_clusters(clusters)

        if all(False for _ in iter(clusters)):
            return (graph, np.arange(graph.branch_count, dtype=int)) if return_branch_reindex_lookup else graph
        ...
        for cluster in clusters:
            ...
            branches_connections = np.isin(graph._branch_list, nodes)
            ...
            if len(branches_to_connect) == 0:
                ...
                graph._branch_list[branches_connections] = n0
        ...
        if len(branches_to_remove) > 0:
            ...
            branch_lookup = graph._delete_branch(branches_to_remove)
        else:
            branch_lookup = np.arange(graph.branch_count, dtype=int)
    ...
    def clear_all_branch_attr(self):
        self._branch_attr = None

    def clear_all_node_attr(self):
    ...
    def nodes(
        self,
        node_ids: NodeIndicesLike | None = None,
        /,
        *,
        ...
        """  # noqa: E501
        nodes_ids = self.as_node_ids(node_ids)
    ...
    def branches(
        self,
        ids: BranchIndicesLike | None = None,
        /,
        *,
    ...
    def jppype_layer(
        ...
        bsplines_out: Optional[list[BSpline]] = None,
        ...
        if self.branch_count > 0:
            if bspline is not None:
                ...
                if bsplines_out is None:
                    bsplines = []
                else:
                    bsplines_out.clear()
                    bsplines = bsplines_out

                if geodata.has_branch_data(bspline) is False:
                    from ..segment_to_graph.geometry_parsing import bsplines_from_curves

                    bsplines.extend(bsplines_from_curves(self))
                else:
                    bsplines += [_.data if _ is not None else BSpline() for _ in geodata.branch_data(bspline)]
                ...
                for i, bspline in enumerate(bsplines):
                    ...
                    bsplines_path.append(bspline.to_path())
                    filler_paths.append([_.to_path() for _ in bspline.filling_curves(n1, n2, smoothing=0.5)])
