...
from .graph_utilities import apply_lookup, apply_node_lookup_on_coordinates, branch_by_nodes_to_adjacency_list, \
    compute_is_endpoints, delete_nodes, distance_matrix, fuse_nodes, merge_nodes_by_distance, \
    merge_nodes_clusters, merge_equivalent_branches, node_rank
...
def seg_to_branches_list(vessel_map: np.ndarray, return_label=False,
    ...
    if merge_small_cycles > 0:
        ...
        cycles = [cycle for cycle, max_dist in zip(cycles, cycles_max_dist, strict=True)
                  if max_dist < merge_small_cycles]
        # - Merge cycles
